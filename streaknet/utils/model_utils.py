...
def get_model_info(model: nn.Module) -> str:
    ...
    signal = torch.zeros((1, 2048), device=next(model.parameters()).device)
    template = torch.zeros((1, 600), device=next(model.parameters()).device)
    flops, params = profile(deepcopy(model), inputs=(signal, template), verbose=False)
    params /= 1e6
    flops /= 1e6
    info = "Params: {:.2f}M, Mflops: {:.2f}".format(params, flops)
    return info
